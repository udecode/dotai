...
const repoRoot = process.env.SYNC_VISION_REPO_ROOT
  ? path.resolve(process.env.SYNC_VISION_REPO_ROOT)
  : process.cwd();
...
const defaultExactInputFiles = new Set([
  'VISION.md',
  'AGENTS.md',
  '.agents/AGENTS.md',
]);
...
const defaultTrackedExts = new Set([
  '.md',
  '.mdx',
  '.mdc',
  '.json',
...
const defaultPatterns = {
  vision: '\\b(VISION\\.md|vision|north[- ]star|taste|doctrine|principle)\\b',
  supervisor: '\\b(sync-vision|autogoal|checkpoint|stopping|handoff|batch|timed|supervisor|loop)\\b',
  behavior: '\\b(selection|caret|typing|paste|clipboard|undo|redo|IME|composition|browser|screenshot|native|workflow|behavior)\\b',
  api_dx: '\\b(API|DX|alias|compat|extension|plugin|transaction|public|docs|migration|developer experience)\\b',
  research: '\\b(research|GitHub|OSS|external|source[- ]mining|benchmark|comparison|discovery)\\b',
  maintainer: '\\b(issue|PR|maintainer|OpenClaw|ledger|checkmark|triage|queue|review)\\b',
  proof: '\\b(proof|oracle|benchmark|metric|test|verify|verification|false positive|evidence)\\b',
};
...
const sourcePathspecs = [
  ...defaultSourcePathspecs,
  ...(config.includePathspecs ?? []),
];
const excludedPathPrefixes = [
  ...defaultExcludedPathPrefixes,
  ...(config.excludePathPrefixes ?? []),
];
const exactInputFiles = new Set([
  ...defaultExactInputFiles,
  ...(config.exactInputFiles ?? []),
]);
const inputPathPrefixes = [
  ...defaultInputPathPrefixes,
  ...(config.inputPathPrefixes ?? []),
];
...
const patterns = Object.fromEntries(
  Object.entries({
    ...defaultPatterns,
    ...(config.patterns ?? {}),
  }).map(([name, pattern]) => [name, new RegExp(pattern, 'i')])
);
...
function isRelevantFile(file) {
  ...
  if (!trackedExts.has(ext) && !exactInputFiles.has(file)) {
    return false;
  }
  return (
